    ...
    function MyAnalysis() {
        ...
        this.invokeFunPre = function (iid, f, base, args, isConstructor, isMethod, functionIid, functionSid) {
            const sinks = {
                ...
                'eval': eval,
                'evalvm1': vm.runInNewContext,
                'evalvm2': vm.runInContext,
                'evalvm3': vm.runInThisContext
            };
            Object.keys(sinks).forEach(function(sinkName) {
                const sinkFn = sinks[sinkName];
                if (f === sinkFn) {
                    console.log(`SINKHIT:${sinkName}`);
                }
            });
