...
export async function loadFilteredPackageList(filteredPackageListPath: Path): Promise<PackageData[]> {
    ...
    try {
        ...
        return data['rows'].map((rawPackage: any) => {
            let thePackage = new PackageData(
                rawPackage['id'] as string,
                Maybe.Just(rawPackage['version'])
            );
...
export async function getPackageListFromPath(packageListPath: Path): Promise<Array<PackageData>> {
    try {
        await fs.access(packageListPath.toString());
    } catch (err) {
        // Package list is not cached, need to re-download
        try {
            // https://replicate.npmjs.com/_all_docs' is throttled so we will use our own mirror
            const response: Response = await (axios as any).get(
                'https://cmu.app.box.com/index.php?rm=box_download_shared_file&shared_name=0a3qlxpyukwmhn6s15j1ee9dz67382mr&file_id=f_881152671310'
            );
    ...
    try {
        ...
        return (packageList as npmPackageList).rows
            ...
            .map(row => new PackageData(row.id, Maybe.Just(row.version)));
    } catch (exn) {
...
export function getPackageListFromPackage(package_description: String): Array<PackageData> {
    var scoped = false;
    if (package_description.startsWith('@')){
        scoped = true;
        package_description = package_description.substring(1);
    }
    var package_name = package_description.split('@')[0];
    var package_version = package_description.split('@')[1];

    if (scoped){
        package_name = '@' + package_name;
    }

    return [new PackageData(package_name, Maybe.Just(package_version))];
}


export function getSafeNameFromPackageName(package_name : String) : string  {
    /*
        Package names can refer to scoped packages
            Scoped packages have an `@` sign at the start, but most importantly, they have a single `/` character in the middle
    */
