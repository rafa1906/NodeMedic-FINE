...
(function (sandbox) {
    ...
    // Do not limit error stack trace
    // (for taint sink errors)
    ...
    config.setFromArgs(process.argv);
    GhostFunction.globalizeGhostFunctions();

    const instrumentation = new Base.Instrumentation(config);
    
    var branches = []; // Coverage info
    var global_branches = []; // Global coverage info (previous one gets reset for every input)
    var code_coverage = 0;
    var global_code_coverage = 0;
    ...
    function MyAnalysis() {
        ...
        function getBranchInfo() {
            var branchInfo = branches[sandbox.sid - 1];
            if (!branchInfo) {
                branchInfo = {};
                branches[sandbox.sid - 1] = branchInfo;
            }
            return branchInfo;
        }

        function getGlobalBranchInfo() {
            var branchInfo = global_branches[sandbox.sid - 1];
            if (!branchInfo) {
                branchInfo = {};
                global_branches[sandbox.sid - 1] = branchInfo;
            }
            return branchInfo;
        }
        ...
        this.invokeFunPre = function (iid, f, base, args, isConstructor, isMethod, functionIid, functionSid) {
            let isExternal = functionSid === null || functionSid === undefined;
            let uniqueID = hash(`${sandbox.sid}${iid}`);
            if (isExternal) {
                var originalScriptPath = sandbox.smap[sandbox.sid]['originalCodeFileName'];
            } else {
                // FunctionSid is only accessible for internal functions
                var originalScriptPath = sandbox.smap[functionSid]['originalCodeFileName'];
            }
            if (originalScriptPath === null || originalScriptPath === undefined) {
                throw Error(`Failed to set original script path. J$.sid is ${sandbox.sid}, functionSid is ${functionSid}`);
            }
            if (f !== undefined) {
                let sourceLocation = sandbox.iidToLocation(sandbox.getGlobalIID(iid));
                let ret = instrumentation.invokeFunPre(f, base, args, isMethod, isConstructor, isExternal, uniqueID, originalScriptPath, code_coverage, global_code_coverage, sourceLocation);
        ...
        this.invokeFun = function (iid, f, base, args, result, isConstructor, isMethod, functionIid, functionSid) {
            let isExternal = functionSid === null || functionSid === undefined;
            let uniqueID = hash(`${sandbox.sid}${iid}`);
            let ret = instrumentation.invokeFun(f, base, args, result, isConstructor, isMethod, isExternal, uniqueID);
        ...
        this.literal = function (iid, val, hasGetterSetter) {
            let ret = instrumentation.literal(val);
        ...
        this.getFieldPre = function (iid, base, offset, isComputed, isOpAssign, isMethodCall) {
            let ret = instrumentation.getFieldPre(base, offset, isMethodCall);
            return {base: ret.base, offset: ret.offset, skip: false};
        };
        ...
        this.getField = function (iid, base, offset, val, isComputed, isOpAssign, isMethodCall) {
            let sourceLocation = sandbox.iidToLocation(sandbox.getGlobalIID(iid));
            let ret = instrumentation.getField(base, offset, val, isMethodCall, sourceLocation);
            return {result: ret.result};
        };
        ...
        this.putFieldPre = function (iid, base, offset, val, isComputed, isOpAssign) {
            let ret = instrumentation.putFieldPre(base, offset, val);
            return {base: ret.base, offset: ret.offset, val: ret.value, skip: false};
        };
        ...
        this.putField = function (iid, base, offset, val, isComputed, isOpAssign) {
            let sourceLocation = sandbox.iidToLocation(sandbox.getGlobalIID(iid));
            let ret = instrumentation.putField(base, offset, val, sourceLocation);
        ...
        this.write = function (iid, name, val, lhs, isGlobal, isScriptLocal) {
            let ret = instrumentation.write(lhs, val);
            return {result: ret.val};
        };
        ...
        this.functionExit = function (iid, returnVal, wrappedExceptionVal) {
            var threwException = (wrappedExceptionVal !== undefined);
        ...
        this.binaryPre = function (iid, op, left, right, isOpAssign, isSwitchCaseComparison, isComputed) {
            let ret = instrumentation.binaryPre(op, left, right);
        ...
        this.binary = function (iid, op, left, right, result, isOpAssign, isSwitchCaseComparison, isComputed) {
            let sourceLocation: string = sandbox.iidToLocation(sandbox.getGlobalIID(iid));
            let ret = instrumentation.binary(op, left, right, result, sourceLocation);
            return {result: ret.result};
        };
        ...
        this.unaryPre = function (iid, op, left) {
            let ret = instrumentation.unaryPre(op, left);
            return {op: op, left: ret.left, skip: false};
        };
        ...
        this.unary = function (iid, op, left, result) {
            let sourceLocation = sandbox.iidToLocation(sandbox.getGlobalIID(iid));
            let ret = instrumentation.unary(op, left, result, sourceLocation);
            return {result: ret.result};
        };
        ...
        this.conditional = function (iid, result) {
            ...
            var globalBranchInfo = getGlobalBranchInfo();
            ...
            let ret = instrumentation.conditional(result);
            return {result: ret.result};
        };
        ...
        this.instrumentCodePre = function (iid, code, isDirect) {
            let isInternal = (iid === undefined);
            let ret = instrumentation.instrumentCodePre(code, isInternal);
            return {code: ret.code, skip: false};
        };
        ...
        this.instrumentCode = function (iid, newCode, newAst, isDirect) {
            let ret = instrumentation.instrumentCode(newCode);
        ...
        this.endExpression = function (iid) {
            /* Uncomment to perform statement (instead of just conditionals) coverage counting
            var branchInfo = getBranchInfo();
