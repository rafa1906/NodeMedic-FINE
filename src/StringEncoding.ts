import { State, taintEntry, PropMap, getTc } from "./State";
...
function encodeStringTaint(x: string, taint_map: number[]) : Either<string, Error> {
    ...
    for (let i = 0; i < codePoints.length; i += chunkSize) {
        ...
        res += String.fromCodePoint(...chunk);
    }
...
export function decodeStringToEntry(s: State, encodedString: string): Either<[string, taintEntry], Error> {
    return F.matchEither(decodeStringTaint(encodedString), {
        Left: ([decodedResult, resultTaintBits]) => {
            ...
            let tEr: taintEntry =  {
                ...
                path: emptyPathNode(decodedResult, getTc(s)),
            };
