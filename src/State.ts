...
export interface TextualContext {
    scriptName: string,
...
export function createTc(
    ...
    return {
        ...
        startColumnNumber: startCol,
        endLineNumber: endLine,
        endColumnNumber: endCol,
    };
}

export function createTcInitial(): TextualContext {
    return createTc('UNKNOWN', -1, -1, -1, -1);
}

export function createTcFromSourceLocation(sourceLocation: string | boolean): TextualContext {
    if (sourceLocation === undefined || sourceLocation === null 
        || sourceLocation === "" || typeof sourceLocation === 'boolean') {
        return createTcInitial();
    }
    let parts = (sourceLocation as string).slice(
        1, (sourceLocation as string).length - 1).split(':');
    return createTc(parts[0], parseInt(parts[1]), parseInt(parts[2]), parseInt(parts[3]), parseInt(parts[4]));
}

export function setTc(s: State, tc: TextualContext): State {
    let sP: State = {
        frame: s.frame,
        Mw: s.Mw,
        Mt: s.Mt,
...
export function getTc(s: State): TextualContext {
    return s.Tc;
}
