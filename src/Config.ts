...
export class Config {
    ...
    // Performs sink propagation
    SINK_PROPAGATION: boolean;
    // Performs sink propagation even if function *isn't* external
    // Requires SINK_PROPAGATION to be true
    AGGRESSIVE_SINK_PROPAGATION: boolean;

    // Generate fake object properties on demand to allow execution
    // to continue with incomplete input objects
    ...
    constructor() {
        ...
        this.SINK_PROPAGATION = true;
        this.AGGRESSIVE_SINK_PROPAGATION = true;
        this.HONEYOBJECTS = false;
    }
    setFromArgs(args: string[]) {
        ...
        args.forEach(function (val) {
            ...
            } else if (val.includes('honeyobjects=')) {
                var result = val.split('=')[1];
                if (result == 'true' || result == 't') {
                    self.HONEYOBJECTS = true;
                } else {
                    self.HONEYOBJECTS = false;
                }
            }
        });
    }
